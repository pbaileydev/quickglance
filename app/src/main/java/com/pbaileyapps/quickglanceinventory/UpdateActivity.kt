...
class UpdateActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val skuField: EditText = findViewById(R.id.update_sku)
        ...
        if (item != null) {
            ...
            if (item.drawable != null) {
                if (ContextCompat.checkSelfPermission(
                    ...
                    findViewById<ImageView>(R.id.update_image).rotation = 90f
                } else {
                ...
                findViewById<ImageView>(R.id.update_delete).setOnClickListener({
                    AlertDialog.Builder(this)
                        .setNegativeButton(
                            ...
                            }).setMessage("Are you sure you want to delete this item?").create()
                        .show()

                })
                findViewById<Button>(R.id.update_button).setOnClickListener {
                    sku = findViewById<EditText>(R.id.update_sku).text.toString()
                    if (TextUtils.isEmpty(name.text) || TextUtils.isEmpty(quantity.text) || TextUtils.isEmpty(
                            needed.text
                        )
                    ) {
                        findViewById<TextInputLayout>(R.id.update_input_needed_layout).setError("Please, fill this field")
                        findViewById<TextInputLayout>(R.id.update_input_quantity_layout).setError("Please, fill this field")
                        findViewById<TextInputLayout>(R.id.update_item_input_layout).setError("Please, fill this field")
                    }
                        ...
                        if (imageUri != null && sku != null) {
                            if (!sku!!.isDigitsOnly()) {
                                ...
                                val itemUpdate: Item = Item(
                                    ...
                                    Integer.parseInt(needed.text.toString()), sku,item.user
                                )
                            ...
                            val itemUpdate: Item = Item(
                                item.id, name.text.toString(), imageUri.toString(),
                                Integer.parseInt(quantity.text.toString()),
                                Integer.parseInt(needed.text.toString()), null,item.user
                            )
                            itemViewModel.updateItem(itemUpdate)
                            startActivity(Intent(this, MainActivity::class.java))
                            finish()
